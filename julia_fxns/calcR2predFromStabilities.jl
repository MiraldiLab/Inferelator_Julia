...
function calcR2predFromStabilities(instabOutMat,trnOutMat,r2OutMat,totSS, geneExprMat, modSizes, instabilitiesDir; xaxisStepSize = 5)
    ...
    modSizes = collect(1:modSizes) # desired model sizes
    ...
    subsampleCutoffs = Float64.(modSizes)  # Edited
    rmInds = []
    ...
    # Convert R2's from a matrix to a vector
    r2_pred = vec(r2_pred)  
    r2_fit = vec(r2_fit)
    r2_predNz = vec(r2_predNz)
    r2_fitNz = vec(r2_fitNz)

    figure(figsize=(3.6, 4))
    pl = plot(instabRangeNet, r2_pred, "b-", label="Predicted") #bo-
    pl = plot(instabRangeNet, r2_fit, "y-", label="Fit") #ys-
    pl = plot(instabRangeNet, r2_predNz, "g--", label="nzPredicted") #gd--
    pl = plot(instabRangeNet, r2_fitNz, "r--", label="nzFit") #r^--
    ...
    legend()
    # Add major and minor gridlines with dashed style
    ...
    # Set axis ticks
    tight_layout()
    savefig((instabilitiesDir * "/r2_pred.pdf"), dpi = 600)
    close()

    totEdges = sum(numParams, dims = 2)/totPredTargs
    totEdges = vec(totEdges)

    figure(figsize=(3.6, 4))
    pl = plot(totEdges,r2_pred, "k-")
    pl = plot(median(numParams, dims = 2),median(r2_pred_byTarg, dims = 2), "b-")
    pl = plot(mean(numParams, dims = 2),mean(r2_pred_byTarg, dims = 2), "r-")
    # Add a dashed black vertical line at the center of the x-axis
    # axvline(x = maximum(modSizes)/2, linestyle="--", color="k")
    xlabel("# of Predictors per Gene", fontsize= axis_title_size)
    ylabel(L"R^2\ \left(1 - \frac{SSE_{model}}{SSE_{mean}}\right)",  fontsize= axis_title_size)
    # legend(["Overall R^2_{pred}","Gene Median R^2_{pred}", "Gene Average R^2_{pred}"])
    ...
    ylim(0,1)
    gca().set_yticks(0:0.2:1)
    gca().set_xticks(0:xaxisStepSize:maximum(modSizes))
    tight_layout()
    savefig((instabilitiesDir * "/model_size.pdf"), dpi = 600)
    close()
    
    @save r2OutMat instabRangeNet numParams r2_pred r2_fit r2_predNz r2_fitNz totEdges r2_pred_byTarg r2_pred_byCond
